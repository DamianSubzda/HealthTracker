  ...
  actions: {
    createLink(
      ...
      const link: ILink = {
        ...
        icon,
        isHidden:
          (isAuthRequired ? !userStore.token : userStore.token && !(name === 'Home' || name === 'About')) ||
    ...
    initializeLinks() {
      this.links = [
        ...
        this.createLink('Planner', 'Trainings Planner', 'fitness_center', true, false),
        this.createLink('Health', 'Health Check', 'health_and_safety', true, false),
        this.createLink('Goals', 'Goals and Progress', 'emoji_events', true, false),
